...
class LubricantesList extends BaseWidget
{
    protected static ?string $panel = 'combustiblesLubricantes';

    protected static ?string $heading = 'LUBRICANTES';
    ...
    {
        return Mantenimientosservices::latest()->take(10)->where('tipo','Lubricante')->get(); // Últimos 5 ingresos
    }
   ...
   public function table(Table $table): Table
    {
        return $table
            ->query(
                Combustibles::query()->select(['id','fecha', 'ingresoLitros', 'origen','egresoLitros','destino'])->where('tipo','Lubricante')->orderBy('fecha', 'desc')
            )
            ->columns([
